    ...
    return (
      <div className='min-h-screen flex flex-col justify-center items-center bg-slate-50 p-4 text-center'>
        <h1 className='text-2xl md:text-3xl font-bold text-slate-800 mb-4'>
          {t('common.noData', 'ไม่พบข้อมูลกิจกรรมนี้')}
        </h1>
        <button
          onClick={() => navigate(-1)}
        ...
        <div className='grid grid-cols-1 lg:grid-cols-4 gap-8 items-start'>
          <div className='lg:col-span-3 bg-white rounded-3xl shadow-md overflow-hidden border border-slate-200'>
            <div className='w-full h-[250px] md:h-[450px] overflow-hidden bg-slate-200 flex justify-center items-center'>
              {newsData.image_src ? (
                <img
                  ...
                  alt={isEn && newsData.title_en ? newsData.title_en : newsData.title}
                  fetchPriority='high'
            ...
            <div className='p-6 md:p-10'>
              <Breadcrumb title={isEn && newsData.title_en ? newsData.title_en : newsData.title} />
              <p className='text-sm text-blue-600 font-bold mb-3'>
                {formatLocalizedDate(newsData.date, isEn)}
              </p>
              <h1 className='text-2xl md:text-4xl font-bold text-slate-800 mb-6 leading-snug'>
                {isEn && newsData.title_en ? newsData.title_en : newsData.title}
              </h1>
          ...
          <div className='lg:col-span-1'>
            <RecommendedSidebar
              ...
              items={newsList}
              basePath='news'
              title={isEn ? 'Other Events' : 'กิจกรรมอื่นๆ จาก ปปง.'}
            />
          </div>
        </div>
      </div>
