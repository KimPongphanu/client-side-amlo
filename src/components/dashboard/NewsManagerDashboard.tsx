...
export default function PRManagerDashboard() {
  ...
  const { newsList: news, fetchNews, updateNews } = useDashboard()
  ...
  // คำนวณจำนวน
  const shownCount = newsList.filter((item: NewsItem) => item.isShow).length
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (activePost === null) {
        await api('/news', { method: 'POST', body: form }) // ปรับตาม API จริง
        await fetchNews()
      } else {
