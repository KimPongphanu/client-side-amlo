      ...
      <div className='flex flex-wrap gap-3 items-center'>
        <div className='relative flex-1 min-w-[180px]'>
          ...
          <input
            ...
            placeholder='ค้นหาชื่อประกาศกิจกรรม...'
...
export default function NewsManagerDashboard() {
  // Bind parameters directly from the centralized dashboard store
  const newsList = useDashboardStore((state) => state.newsList)
  const fetchNews = useDashboardStore((state) => state.fetchNews)
  const createNews = useDashboardStore((state) => state.createNews)
  const updateNews = useDashboardStore((state) => state.updateNews)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (activePost === null) {
        ...
        await updateNews(formData.id, form)
      }
          ...
          >
            <div className='px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0'>
              <h2 className='text-base md:text-md font-bold text-slate-800'>
                {activePost === null
                  ? '✨ สร้างประกาศข่าวสารกิจกรรมใหม่'
                  : `✏️ แก้ไขรายละเอียดกิจกรรม: ID ${formData.id}`}
              </h2>
                ...
                <div className='max-w-3xl mx-auto space-y-6 pb-4 text-sm font-medium'>
                  ...
                  <div className='space-y-1.5 border-b border-slate-100 pb-5'>
                    ...
                    <input
                      ...
                      placeholder='กรอกระบุหัวข้อรายละเอียดงานจัดกิจกรรม...'
                  ...
                  <div className='space-y-1.5 border-b border-slate-100 pb-5'>
                    <label className='block font-bold text-slate-700'>
                      รูปภาพหน้าปกกิจกรรม (Cover Image)
                    </label>
                  ...
                  <div className='space-y-1.5'>
                    <label className='block font-bold text-slate-700'>
                      เนื้อหารายละเอียดกิจกรรมฉบับเต็ม (Content Details)
                    </label>
