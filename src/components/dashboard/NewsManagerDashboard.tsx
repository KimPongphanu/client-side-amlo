...
// ==========================================
const MiniNewsPreview = ({ data }: { data: NewsItem | null }) => {
  if (!data) return null
    ...
    <div className='bg-white h-full overflow-y-auto custom-scrollbar border border-slate-200 rounded-xl relative shadow-sm'>
      ...
      <div className='p-4 md:p-6 pb-8'>
        ...
        <h1 className='text-base md:text-lg font-bold text-slate-800 mb-3 leading-snug'>
          {data.title || 'พิมพ์หัวข้อกิจกรรม...'}
        </h1>
        <hr className='border-slate-100 mb-4' />
        <div
          className='text-slate-600 text-sm leading-relaxed ql-rendered'
          dangerouslySetInnerHTML={{ __html: data.content || data.description || 'พิมพ์เนื้อหากิจกรรม...' }}
        />
      </div>
...
const ToggleSwitch = ({ checked, onChange, itemId }: ToggleSwitchProps) => (
  <label className='relative inline-flex items-center cursor-pointer gap-2'>
    ...
    <input type='checkbox' checked={checked} onChange={onChange} className='sr-only peer' id={`toggle-news-${itemId}`} />
...
export default function NewsManagerDashboard() {
  // ดึงข้อมูล newsList (กิจกรรมและประกาศ) จาก NewsContext
  const context = useContext(NewsContext);
  const newsList = useMemo(() => context?.newsList || [], [context?.newsList]);
  const setNewsList = context?.setNewsList || (() => {});
  ...
  const handleToggleShow = (id: number, e: React.MouseEvent | React.ChangeEvent) => {
    e.stopPropagation()
    setNewsList((prev) => prev.map((p) => (p.id === id ? { ...p, isShow: !p.isShow } : p)))
  }
  ...
  const handleSave = () => {
    if (!formData || !formData.title.trim()) {
      Swal.fire({ icon: 'error', title: 'ข้อมูลไม่ครบ', text: 'กรุณาระบุหัวข้อกิจกรรม' })
      if (mobileView === 'preview') setMobileView('form')
      return
    }
    Swal.fire({ title: 'กำลังบันทึกข้อมูล...', timer: 800, didOpen: () => Swal.showLoading() }).then(() => {
      if (activePost === null) setNewsList((prev) => [formData, ...prev])
      else setNewsList((prev) => prev.map((p) => (p.id === formData.id ? formData : p)))
      handleCloseModal()
  ...
  return (
    <div className='bg-slate-100 min-h-screen p-4 md:p-8 font-sans text-slate-800'>
      ...
      <div className='max-w-7xl mx-auto mb-5 flex flex-col md:flex-row justify-between items-start md:items-end gap-4'>
        <div>
          <h1 className='text-2xl font-bold mb-2'>จัดการกิจกรรมและประกาศ</h1>
          <div className='flex items-center gap-4 text-sm'>
            <span className='text-slate-600'>ทั้งหมด: <span className='font-bold text-slate-900'>{newsList.length}</span></span>
      ...
      <div className='max-w-7xl mx-auto'>
        <FilterBar search={search} onSearch={setSearch} status={statusFilter} onStatus={setStatusFilter} sort={sortOrder} onSort={setSortOrder} view={viewMode} onView={setViewMode} totalCount={newsList.length} filteredCount={filteredList.length} />
      </div>
          ...
          {filteredList.map((item) => (
            ...
            >
              ...
              <div className='flex-1'>
                <p className='text-xs text-blue-600 font-bold mb-1'>{item.date}</p>
        ...
        <div onClick={handleCloseModal} className='fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4'>
          <div onClick={(e) => e.stopPropagation()} className='bg-white w-full max-w-6xl h-[95vh] md:h-[90vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up'>
            ...
            <div className='px-4 md:px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 shrink-0'>
              <h2 className='text-base md:text-lg font-bold text-slate-800'>{activePost === null ? 'สร้างกิจกรรมใหม่' : `แก้ไขข้อมูล: ID ${formData.id}`}</h2>
              ...
              <div className={`flex-1 overflow-y-auto custom-scrollbar bg-white p-4 md:p-8 ${mobileView === 'preview' ? 'hidden lg:block' : 'block'}`}>
                <div className='max-w-3xl mx-auto space-y-6 md:space-y-8 pb-4'>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-4 gap-2 md:gap-4 items-start border-b border-slate-100 pb-6 md:pb-8'>
                    <label className='text-sm font-bold text-slate-700 md:pt-2'>หัวข้อกิจกรรม <span className='text-red-500'>*</span></label>
                    <div className='md:col-span-3'>
                      <input type='text' value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder='ระบุหัวข้อกิจกรรมหรือประกาศ...' className='w-full border border-slate-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500' />
                    </div>
                  </div>
            ...
            <div className='px-4 md:px-6 py-4 border-t border-slate-200 bg-white flex justify-between md:justify-end gap-3 shrink-0'>
              ...
              <button onClick={handleSave} className='px-6 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 shadow-sm'>บันทึกกิจกรรม</button>
            </div>
          </div>
        </div>
      )}
